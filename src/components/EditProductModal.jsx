...
export default function EditProductModal({ product, onSave, onDelete, onClose }) {
  const [form, setForm] = useState({ ...product });

  const handleChange = (key, value) => {
    setForm((prev) => {
      const updated = { ...prev, [key]: value };
      // Auto-calculate factory price: 出厂价格 = 料价 × 规格 × 克重
    ...
    <div className="fixed inset-0 bg-black/50 z-[100] flex items-end sm:items-center justify-center">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-100 flex-shrink-0">
          ...
          <h2 className="font-bold text-sm">编辑产品</h2>
          <div className="flex items-center gap-3">
            {onDelete && (
              <button onClick={() => { if (confirm('确定删除该产品？')) onDelete(product.id); }} className="text-red-500">
                <Trash2 size={16} />
              </button>
            )}
